...
export function EnhancedAvatar3D({ emotion, voiceActive, onVoiceCommand }: EnhancedAvatar3DProps) {
  ...
  useEffect(() => {
    if (voiceActiveState && onVoiceCommand) {
  ...
  }, [voiceActiveState, onVoiceCommand])
  ...
  return (
    <div className="w-full h-full bg-gradient-to-b from-slate-900/50 to-black/50 rounded-lg overflow-hidden relative">
      <Canvas
        frameloop="demand"
        dpr={[1, 1]}
      ...
      >
        ...
        <AvatarHead emotion={emotion} speaking={speaking} />
